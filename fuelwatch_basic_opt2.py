...
def gen_html_table_data(cleaned_xml_data):
    """
...
def gen_html(table_content_data):
...
def run():
    ...
    for each_url in url_list:
        ...
        # 4. Create the table content
        table_content_data_temp = gen_html_table_data(cleaned_fuelwatch_xml_data)

        if table_content_data_temp:
            table_content_data = table_content_data + table_content_data_temp

        # print(table_content_data)

    # 5. Create the HTML output
    gen_html(table_content_data)
...
- Is not sorting all information by price, only each URLs xml data is sorted.
- Suburb filtering is not working
- If you leave one of the filtering sections blank, it fails
